...
/// 聚合根保存事件
/// </summary>
public record class AggregateRootSavedEvent<TAggregateRoot>(TAggregateRoot AggregateRoot) : INotification where TAggregateRoot : IAggregateRoot;

public class AggregateRootSavedEvent
{
    ...
    public static INotification GetNotificationFrom(IAggregateRoot aggregateRoot)
    {
        ...
        if (_cache == null || _cache.TryGetValue(realType, out var newer) == false)
        {
            ...
            // 获取构造方法
            var constructor = typeof(AggregateRootSavedEvent<>).MakeGenericType(realType).GetConstructors()[0];
